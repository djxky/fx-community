...
test('创作者中心独立地址会恢复到创作者中心页面', () => {
  assert.deepEqual(getViewRouteFromSearch('?view=creator'), { view: 'creator' })
})
...
test('进入 AI 教学工坊会生成可分享的独立地址，离开时清除页面参数', () => {
  ...
  assert.equal(buildViewUrl('/fx-community/', 'creator'), '/fx-community/?view=creator')
