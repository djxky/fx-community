...
// 我的主页 · 专辑（专题）管理：示例作品 / 示例专辑 + 纯函数操作 + 渲染片段。
...
export const OWNER_WORKS = [
  { id: 'taohuayuan', title: '副本《古文沉浸式漫游·桃花源记》', type: '应用', use: 31, cover: 'linear-gradient(135deg,#2A3A32,#141F1B)' },
  { id: 'layered-reading', title: 'AI 分层阅读', type: '应用', use: 56, cover: 'linear-gradient(135deg,#1C2823,#141F1B)' },
  { id: 'luoluobi', title: '落落笔 · AI 写作编辑器', type: '应用', use: 47, cover: 'linear-gradient(135deg,#1C2823,#141F1B)' },
  { id: 'class-pet', title: '改编《班级宠物乐园·一起进化吧！》', type: '应用', use: 24, cover: 'linear-gradient(135deg,#B87333,#8A5A24)' },
  { id: 'feihualing', title: '诗云 · 飞花令', type: '应用', use: 12, cover: 'linear-gradient(135deg,#2A3A32,#141F1B)' },
  { id: 'picture-book-questions', title: '教师生成绘本题目', type: '技能', use: 18, cover: 'linear-gradient(135deg,#1C2823,#141F1B)' },
]
...
export function albumSummary(album, works = OWNER_WORKS) {
  const lessons = albumLessons(album, works)
  const types = [...new Set(lessons.map((w) => w.type))].join('+')
  const use = lessons.reduce((sum, w) => sum + w.use, 0)
  return `${types} · ${lessons.length} 讲 · ${formatCount(use)} 使用`
}

export function validateAlbumInfo(albums, draft) {
  const name = draft.name.trim()
  if (!name) return '请填写专辑名称'
  if ([...name].length > ALBUM_NAME_MAX) return `专辑名称最多 ${ALBUM_NAME_MAX} 个字`
  if (albums.some((a) => a.id !== draft.id && a.name.trim() === name)) return '已有同名专辑，换个名字吧'
  return ''
}
...
export function renderAlbumList(albums, openMenuId = '') {
  return albums.map((album, i) => {
    ...
    const menu = menuOpen ? `<div class="st-album-menu" role="menu">
      <button type="button" role="menuitem" data-album-act="edit" ${TRACK('albumEdit', '编辑专辑', '无')}>编辑专辑</button>
      <button type="button" role="menuitem" data-album-act="up"${i === 0 ? ' disabled' : ''} ${TRACK('albumMove', '调整专辑顺序', 'direction#STRING')}>上移</button>
      <button type="button" role="menuitem" data-album-act="down"${i === albums.length - 1 ? ' disabled' : ''} ${TRACK('albumMove', '调整专辑顺序', 'direction#STRING')}>下移</button>
      <button type="button" role="menuitem" class="is-danger" data-album-act="delete" ${TRACK('albumDelete', '删除专辑', '无')}>删除专辑</button>
    </div>` : ''
    const lessons = albumLessons(album).map((w, n) => `<div class="st-album-lesson">
        ...
        <div class="st-album-ls">${escapeHtml(w.type)}</div>
      </div>`).join('')
    return `<div class="st-album nav-res" data-album-id="${escapeHtml(album.id)}" ${TRACK('topicCard', '点击专题卡', 'position#INT')}>
    <div class="st-album-hd">
      ...
      <span class="st-album-more-wrap">
        <button type="button" class="st-album-more" data-album-act="menu" aria-label="管理专辑" aria-haspopup="menu" aria-expanded="${menuOpen}" ${TRACK('albumMenu', '打开专辑管理菜单', '无')}>···</button>${menu}
      </span>
...
export function renderLessonOrder(workIds, works = OWNER_WORKS) {
  ...
  return lessons.map((w, i) => `<div class="album-order-row" data-work-id="${escapeHtml(w.id)}">
    ...
    <button type="button" class="album-order-btn" data-lesson-act="remove" aria-label="移出专辑">✕</button>
  </div>`).join('')
}
